...
export class ProgramManageComponent implements AfterViewInit {
  ...
  onFacultyChange(): void {
    ...
    const programs = this.getProgramsForFaculty();
    if (programs.length > 0) {
      this.selectProgram(programs[0].code);
    }
  }
  ...
  onTabChange(event: MatTabChangeEvent): void {
    const program = this.getProgramsForFaculty()[event.index];
    if (program) {
      this.selectProgram(program.code);
    }
  }
