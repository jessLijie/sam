...
export class ViewApplicantsComponent implements OnInit {
  ...
  @ViewChild('stpmFileInput') stpmFileInput!: ElementRef;
  scanPreUResult(): void {
    // this.preUResults.push(this.fb.group({
    //   subject: ['', Validators.required],
    //   grade: ['', Validators.required]
    // }));
    this.stpmFileInput.nativeElement.click(); // triggers the file input
  }
  ...
  onStpmFileSelected(event: Event) {
    this.preUResults.clear();
    const input = event.target as HTMLInputElement;
    const file = input.files?.[0];
    if (!file) return;
    ...
    this.http.post<any>(`${this.apiUrl}/scan/stpm`, formData
      // ,{ responseType: 'text' as 'json' }
    ).subscribe({
      next: (res) => {
